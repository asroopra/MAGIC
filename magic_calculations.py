...
def calculate_padj(df):
    ...
    df["padj"] = np.where(df["padj"] >= 1.0, 1.0, df["padj"])
...
def get_d_arg(m_vals, q_vals):
    ...
    mHistogram = np.histogram(m_vals, bins=len(set(m_vals)), density=True)
    mHist = mHistogram[0]
    norm_mHist = mHist / np.max(mHist)
    mBins = mHistogram[1]

    # qHist = query histogram
    qHistogram = np.histogram(
        q_vals, bins=len(set(m_vals)), density=True, range=(min(m_vals), max(m_vals))
    )
    qHist = qHistogram[0]
